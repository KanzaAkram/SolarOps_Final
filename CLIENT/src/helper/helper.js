...
export async function SignUp(credentials) {
  try {
    const { data: { msg } } = await Axios.post('https://solar-ops-final.vercel.app/api/register', credentials);
...
console.log(user);
    ...
    const { data: { msg } } = await Axios.post('https://solar-ops-final.vercel.app/api/visualizations', {
      email: user,
...
export async function GetUserLocations() {
  try {
    ...
    const { data } = await Axios.get('https://solar-ops-final.vercel.app/api/visualizations', {
...
export async function DeleteLocation(location_name) {
  try {
      ...
      const { data: { msg } } = await Axios.delete('https://solar-ops-final.vercel.app/api/visualizations?location_name=${encodeURIComponent(location_name)}', {
          headers: {
              "user-email": user,
          }
      });
